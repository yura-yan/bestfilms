...
class Auth extends MY_Controller
{
	...
	function login()
	{
		if ( ! $this->dx_auth->is_logged_in())
		{
			...
			if ($val->run() AND $this->dx_auth->login($val->set_value('username'), $val->set_value('password'), $val->set_value('remember')))
			{
				...
				if ($this->input->post('uri') == "auth/register" || $this->input->post('uri') == "auth/activate/(:any)") {
					redirect('', 'location');
				} else {
				redirect($this->input->post('uri'), 'location');
				}	
			}
			else
			{
				...
				elseif ($this->dx_auth->get_auth_error()) {
	...
	function register()
	{
		...
		{	
			...
			{	
				...
				{
					$data['auth_message'] = 'Вы успешно зарегистрировались. Проверьте адрес электронной почты, чтобы активировать свой аккаунт.';
					//$this->session->set_flashdata('in',1);
				}
				else
				{					
	...
	function activate()
	{
		...
		{
			$data['auth_message'] = 'Ваш аккаунт был успешно активирован.';//.anchor(site_url($this->dx_auth->login_uri), 'Login');
		...
		{
			$data['auth_message'] = 'Код активации, который вы ввели, был неверным. Пожалуйста, проверьте свою электронную почту еще раз.';
